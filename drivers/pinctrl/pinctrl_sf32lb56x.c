...
#define DT_DRV_COMPAT sifli_sf32lb56x_pinmux
...
struct sf32lb56x_pinctrl_config {
	uintptr_t pad_sa;
	uintptr_t pad_pa;
	uintptr_t pad_pb;
	uintptr_t cfg;
...
static int pinctrl_configure_pin(pinctrl_soc_pin_t pin)
{
	...
	const struct sf32lb56x_pinctrl_config *config = dev->config;
	uintptr_t pad;
	uint8_t pinr_offset;
	uint32_t val;
	...
	if (port == SF32LB_PORT_PB) {
		/*
		 * LPSYS PINMUX (PAD_PBxx): bits 4-6 (PE, PS, IE) and bits 0-2
		 * (FSEL, 3-bit) are at the same positions as HPSYS. No DS or SR
		 * bits exist on LPSYS pins — drive strength is not configurable.
		 * PINR is HPSYS-CFG-specific and does not apply here.
		 */
		pad = config->pad_pb + pad_num * 4U;
		val = sys_read32(pad);
		val &= ~(SF32LB_FSEL_MSK | SF32LB_PE_MSK | SF32LB_PS_MSK | SF32LB_IE_MSK);
		val |= (pin & (SF32LB_FSEL_MSK | SF32LB_PE_MSK | SF32LB_PS_MSK | SF32LB_IE_MSK));
		sys_write32(val, pad);
		return 0;
	}
...
static int sf32lb56x_pinctrl_init(const struct device *dev)
{
	const struct sf32lb56x_pinctrl_config *const config = dev->config;
...
static const struct sf32lb56x_pinctrl_config config = {
	.pad_sa = DT_INST_REG_ADDR_BY_NAME(0, pad_sa),
	.pad_pa = DT_INST_REG_ADDR_BY_NAME(0, pad_pa),
	.pad_pb = DT_INST_REG_ADDR_BY_NAME(0, pad_pb),
	.cfg = DT_REG_ADDR(DT_INST_PHANDLE(0, sifli_cfg)),
	.clock = SF32LB_CLOCK_DT_INST_SPEC_GET(0),
};

DEVICE_DT_INST_DEFINE(0, sf32lb56x_pinctrl_init, NULL, NULL, &config, PRE_KERNEL_1,
